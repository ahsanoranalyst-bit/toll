...
if not st.session_state['logged_in']:
    ...
    st.write("AI-powered logistics dashboard. Fully automated toll risk and time zone tracking.")
    ...
    if st.session_state['calculate_pressed']:
        geolocator = Nominatim(user_agent="automated_dispatcher_v8")
        locations_data = []
        coordinates = []
        states_visited = set()
        ...
        if len(coordinates) >= 2:
            ...
            if total_miles < 50.0:
                ...
                # Rule 3: Dynamic Distance Scaling Factor (Shorter regional trips pay a fraction of the max toll cap)
                # Assumes 600 miles maxes out the complete regional turnpike loop
            ...
            with col1:
                st.metric(label=" Eustimated Route Mileage", value=f"{total_miles:,.1f} Miles")
